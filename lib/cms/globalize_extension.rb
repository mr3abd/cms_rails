        ...
        define_method :translated_scope do |*attrs|
          ...
          if !self.respond_to?(:translated)
            scope :translated, ->(locale = I18n.locale, attributes_to_check = nil) {
              attrs = attributes_to_check.present? ? Array.wrap(attributes_to_check).select(&:present?) : nil
              attrs = self.class_variable_get(:@@_translated_scope_attrs) if attrs.blank?
